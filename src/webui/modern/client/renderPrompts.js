export function renderClientRenderPrompts() {
  return `
function renderPrompts(prompts, defId) {
  const wrap = $("prompts");
  wrap.innerHTML = "";
  const ids = prompts.map((p) => String((p && p.id) || "")).filter(Boolean);
  renderSelectOptions($("fDefaultPrompt"), ids.length ? ids : [""], defId || ids[0] || "");
  let idx = 0;
  for (const p of prompts) {
    const item = document.createElement("details");
    item.className = "details prompt-item";
    ...
    l3.textContent = I.labelSystemPrompt;
    ...
    const updateSummary = () => {
      ...
      sumTitle.textContent = namev || idv || I.wordPrompt;
...
function refreshDefaultPromptOptions() {
  const ids = [...document.querySelectorAll('.prompt-item [data-field="id"]')]
    .map((i) => String(i.value || "").trim())
    .filter(Boolean);
  const current = $("fDefaultPrompt").value;
  renderSelectOptions($("fDefaultPrompt"), ids.length ? ids : [""], ids.includes(current) ? current : ids[0] || "");
}
`;
}
